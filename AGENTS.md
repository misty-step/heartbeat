...
- `bun install` (Node >=18.17).
- `bun dev` runs Next (Turbopack) + Convex together; or split via `bun dev:next` and `bun dev:convex`.
- `bun lint`, `bun type-check` must pass before PR.
- `bun build` → production bundle; `bun start` serves it.
- Convex ops: `bunx convex dev` (generates `_generated/` types), `bunx convex deploy`, `bunx convex dashboard`.
...
- Always run `bun lint` + `bun type-check`; note any skipped cases in PR.
