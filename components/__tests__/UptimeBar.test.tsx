...
describe("UptimeBarSimple", () => {
  ...
  test("fills all bars when percentage is 100", () => {
    const { container } = render(
      <UptimeBarSimple percentage={100} days={10} />,
    );
    const filledBars = container.querySelectorAll(".bg-up");
  ...
  test("fills no bars when percentage is 0", () => {
    ...
    // Empty bars use subtle border color
    const allBars = container.querySelectorAll(".flex-1.h-6");
    const emptyCount = Array.from(allBars).filter(
      (bar) => !bar.className.includes("bg-up"),
    ).length;
