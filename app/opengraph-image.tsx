...
export default async function Image() {
  ...
  const fontEntries: { name: string; data: ArrayBuffer; weight: 700 | 800; style: "normal" }[] = [];
  try {
    ...
    await Promise.all(
      ttfMatches.map(async ([, weight, url]) => {
        ...
        if (w !== 700 && w !== 800) return;
        const data = await fetch(url).then((r) => r.arrayBuffer());
        fontEntries.push({ name: "Plus Jakarta Sans", data, weight: w as 700 | 800, style: "normal" });
      }),
    );
  } catch {
