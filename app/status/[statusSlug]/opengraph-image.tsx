...
export default async function Image({
  ...
  // Fetch uptime stats — failure degrades gracefully (no percentage shown)
  let uptimePercentage: number | null = null;
  try {
    const uptimeStats = await fetchPublicQuery(
      api.checks.getPublicUptimeStats,
      {
    ...
    // Non-fatal: OG image renders without uptime percentage
  }

  // Theme colors
  const themeId = (monitor.theme as ThemeId) ?? DEFAULT_THEME;
  const theme = THEMES[themeId] ?? THEMES[DEFAULT_THEME];
  const [accent, statusUp, statusWarn] = theme.colors;
  ...
  const fonts = await loadPlusJakartaSans([600, 700, 800]);
